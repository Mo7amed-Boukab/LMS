...
const courses = [
    {
        ...
        price: 49.99,
        status: "Published",
        image: "https://images.unsplash.com/photo-1561070791-2526d30994b5?w=100&auto=format&fit=crop&q=60",
    },
    {
        id: 2,
        ...
        price: 89.99,
        status: "Published",
        image: "https://images.unsplash.com/photo-1526374965328-7f61d4dc18c5?w=100&auto=format&fit=crop&q=60",
    },
    {
        id: 3,
        ...
        price: 29.99,
        status: "Draft",
        image: "https://images.unsplash.com/photo-1460925895917-afdab827c52f?w=100&auto=format&fit=crop&q=60",
    },
    {
        id: 4,
        ...
        price: 129.99,
        status: "Published",
        image: "https://images.unsplash.com/photo-1551288049-bebda4e38f71?w=100&auto=format&fit=crop&q=60",
    },
    {
        id: 5,
        ...
        price: 59.99,
        status: "Archived",
        image: "https://images.unsplash.com/photo-1507537297725-24a1c434c67b?w=100&auto=format&fit=crop&q=60",
    },
    {
        id: 6,
        ...
        price: 99.99,
        status: "Published",
        image: "https://images.unsplash.com/photo-1633356122544-f134324a6cee?w=100&auto=format&fit=crop&q=60",
    },
];

const categories = ["All Categories", "Design", "Development", "Marketing", "Data", "Business", "Finance"];
                ...
                <div className="absolute top-full left-0 right-0 mt-1 bg-white border border-gray-100 rounded shadow-lg z-50 py-1 max-h-60 overflow-y-auto">
                    {options.map((option) => (
                        <button
                            ...
                            className={`w-full text-left px-3 py-2 text-sm hover:bg-gray-50 flex items-center justify-between ${value === option ? "text-red-600 font-medium bg-red-50" : "text-gray-600"
                                }`}
                        >
                            {option}
        ...
        <>
            ...
            <div className="p-6 max-w-7xl mx-auto space-y-6">
                ...
                <div className="flex flex-col md:flex-row gap-4 justify-between items-start md:items-center">
                    ...
                    {/* Create Button */}
                   <Link href="/teacher/courses/create"> <button className="flex items-center gap-2 bg-red-700 hover:bg-red-800 text-white px-4 py-2 rounded-sm font-medium text-sm transition-colors shadow-sm w-full md:w-auto justify-center">
                        <Plus size={16} />
                ...
                <div className="bg-white rounded-md border border-gray-100 overflow-hidden">
                        ...
                        <table className="w-full">
                            <thead className="bg-gray-50/50">
                                <tr>
                                    ...
                                    <th className="text-left py-3 px-5 text-xs font-semibold text-gray-400 uppercase tracking-wider">Enrolled</th>
                                    <th className="text-left py-3 px-5 text-xs font-semibold text-gray-400 uppercase tracking-wider">Price</th>
                                    <th className="text-left py-3 px-5 text-xs font-semibold text-gray-400 uppercase tracking-wider">Status</th>
                                    <th className="text-right py-3 px-5 text-xs font-semibold text-gray-400 uppercase tracking-wider">Actions</th>
                                </tr>
                            </thead>
                            <tbody className="divide-y divide-gray-50">
                                {filteredCourses.map((course) => (
                                    <tr key={course.id} className="hover:bg-gray-50/50 transition-colors group">
                                        <td className="py-4 px-5">
                                            <div className="flex items-center gap-3">
                                                ...
                                                <div>
                                                    ...
                                                    <span className="text-xs text-gray-400">ID: #C-{1024 + course.id}</span>
                                                </div>
                                            </div>
                                        </td>
                                        <td className="py-4 px-5">
                                            ...
                                            {course.students > 0 ? (
                                                <div className="flex items-center gap-1.5">
                                                    <div className="flex -space-x-2">
                                                        {[...Array(Math.min(3, course.students))].map((_, i) => (
                                                            <div
                                                                key={i}
                                                ...
                                                <span className="text-xs text-gray-400 italic">No enrollments yet</span>
                                            )}
                                        </td>
                                        <td className="py-4 px-5">
                                        ...
                                        <td className="py-4 px-5">
                                            <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-medium ${course.status === 'Published'
                                                ? 'text-green-700'
                                                : course.status === 'Draft'
                                                    ? 'text-red-700'
                                                    : 'text-gray-600'
                    ...
                    <div className="p-4 border-t border-gray-100 flex items-center justify-between">
                        <span className="text-xs text-gray-500">Showing {filteredCourses.length} of {courses.length} courses</span>
                        <div className="flex gap-2">
                            <button className="px-3 py-1.5 text-xs border border-gray-200 rounded-md text-gray-500 hover:bg-gray-50 transition-colors">Previous</button>
                            <button className="px-3 py-1.5 text-xs border border-gray-200 rounded-md text-gray-500 hover:bg-gray-50 transition-colors">Next</button>
                        </div>
                    </div>
                </div>
            </div>
