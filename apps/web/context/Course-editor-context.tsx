...
interface CourseEditorContextType {
  ...
  createLesson: (
    moduleId: string,
    data: { title: string; type: "VIDEO" | "PDF"; contentUrl: string }
  ) => Promise<void>;
  updateLesson: (
    lessonId: string,
...
const CourseEditorContext = createContext<CourseEditorContextType | undefined>(
  undefined
);
...
export function CourseEditorProvider({
  ...
  const refreshModules = async () => {
    try {
      const modulesData =
        await teacherCourseService.getModulesByCourse(courseId);

      const modulesWithLessons = await Promise.all(
        modulesData.map(async (module) => {
          const lessons = await teacherCourseService.getLessonsByModule(
            module._id
          );
  ...
  const reorderModules = async (moduleIds: string[]) => {
    // Optimistic update
    const reorderedModules = [...modules].sort(
      (a, b) => moduleIds.indexOf(a._id) - moduleIds.indexOf(b._id)
    );
  ...
  const reorderLessons = async (moduleId: string, lessonIds: string[]) => {
    ...
    const updatedModules = modules.map((m) => {
      if (m._id === moduleId && m.lessons) {
        const reorderedLessons = [...m.lessons].sort(
          (a, b) => lessonIds.indexOf(a._id) - lessonIds.indexOf(b._id)
        );
